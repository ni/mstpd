...
static bool PRSM_to_DISCARD(port_t *prt, bool dry_run)
{
    if(dry_run)
    {
        return (prt->PRSM_state != PRSM_DISCARD)
               ...
               || (epoll_timer_which_second(&prt->edgeDelayWhile) != prt->bridge->Migrate_Time)
...
static bool PRSM_run(port_t *prt, bool dry_run)
{
    ...
    if((prt->rcvdBpdu || (epoll_timer_which_second(&prt->edgeDelayWhile) != prt->bridge->Migrate_Time))
       && !prt->portEnabled)
    {
